...
function SmartAllowanceDashboard() {
    ...
    return (
        <div className="min-h-screen bg-gray-50 dark:bg-gray-900 text-gray-800 dark:text-gray-200 transition-colors duration-300 p-8">
            <div className="max-w-5xl mx-auto bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-8 transition-colors duration-300">
                ...
                {walletAddress && (
                    <div className="grid md:grid-cols-2 gap-8">
                        <div className="p-6 bg-gray-100 dark:bg-gray-700 rounded-lg shadow-sm border border-gray-200 dark:border-gray-600 transition-colors duration-300">
                            <h2 className="text-xl font-semibold mb-4 text-gray-900 dark:text-white">💰 Create Allowance Plan</h2>
                            <input
                                type="text"
                                placeholder="Beneficiary Address"
                                value={beneficiary}
                                onChange={(e) => setBeneficiary(e.target.value)}
                                className="w-full mb-3 p-2 border rounded bg-white dark:bg-gray-800 text-gray-800 dark:text-gray-200 transition-colors duration-300"
                            />
                            <input
                                type="text"
                        ...
                        <div className="p-6 bg-gray-100 dark:bg-gray-700 rounded-lg shadow-sm border border-gray-200 dark:border-gray-600 transition-colors duration-300">
                            ...
                            <button
                                onClick={fetchPlan}
                                className="mb-3 px-4 py-2 bg-blue-600 text-white rounded transition-colors duration-300"
                            >
                                Refresh Plan
                            </button>
                            ...
                            {plan ? (
                                <div className="text-sm space-y-2">
                                    ...
                                    <button
                                        onClick={handleClaim}
                                        disabled={loading}
                                        className="mt-4 w-full bg-purple-600 hover:bg-purple-700 text-white py-2 rounded-lg transition-colors duration-300"
                                    >
                                        {loading ? "Claiming..." : "Claim Allowance"}
                                    </button>
                                </div>
                            ) : (
                                <p className="text-gray-500 dark:text-gray-300">No active plan found.</p>
                            )}
                        </div>
                    </div>
                )}
